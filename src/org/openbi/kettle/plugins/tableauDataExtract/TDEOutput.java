  ...
  public void openNewFile(String baseFilename) throws KettleException {
	...
	private void createParentFolder(String filename) throws Exception
	{
		...
		try
		{
    		...
    		if(parentfolder.exists()){
    			if (isDetailed()) logDetailed(BaseMessages.getString(PKG, "TDEOutput.Log.ParentFolderExist",parentfolder.getName()));
    		}else{
    			if (isDetailed()) logDetailed(BaseMessages.getString(PKG, "TDEOutput.Log.ParentFolderNotExist",parentfolder.getName()));
    			if(meta.isCreateParentFolder()){
    				parentfolder.createFolder();
    				if (isDetailed()) logDetailed(BaseMessages.getString(PKG, "TDEOutput.Log.ParentFolderCreated",parentfolder.getName()));
    			}else{
	...
	private void checkFileExists(String filename) throws Exception
	{
		...
		FileObject parentfolder=null;
			...
			{
				file.delete();
				if(isDetailed()) logDetailed(BaseMessages.getString(PKG,"TDEOutput.Log.FileDeleted",filename));
			}  else
			{
				if(isDetailed()) logDetailed(BaseMessages.getString(PKG,"TDEOutput.Log.FileDoesNotExist"));
			}
